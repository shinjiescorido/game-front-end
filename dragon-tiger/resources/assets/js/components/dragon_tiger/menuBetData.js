import { createCardSprite, numberWithCommas } from '../../factories/factories';
	...
	instance = instance || new blu.Component({
		...
		main() {
			...
			this.gameHistoryEl = [
				...
				{"table_name" : window.language.menu.cardvaluescaps, "table_width" : 200},
		...
		setHistoryData(data) {
			...
			for (var i = 0; i < this._gameHistory.data.length; i++) {
		        ...
		        this._histCards[i].dragon.x = 250 - (cardwidth + 15);

		        this._histCards[i].tiger.gotoAndPlay("C" + JSON.parse(record.game_info).tiger)
		        this._histCards[i].tiger.x = 265;
        ...
        if(this._gameHistory.data[i].status && this._gameHistory.data[i].status.toLowerCase() === 'w') {
          ...
          voidText.x = voidShape.getBounds().width - 236;
		...
		showDetails(roundNum, roundDate, mWidth, table) {
			...
			let detailHeaderTxt = new createjs.Text(window.language.menu.winningresultcaps, 'bold 20px lato-regular', '#2b2b2b');
			...
			let tblHeaderBetType = new createjs.Text(window.language.menu.betcaps +' '+ window.language.menu.typecaps, 'bold 16px lato-regular', '#242021');
			...
			let tblHeaderBets = new createjs.Text(window.language.menu.betscaps, 'bold 16px lato-regular', '#242021');
			...
			let tblHeaderWin = new createjs.Text(window.language.menu.payoutcaps, 'bold 16px lato-regular', '#242021');
			...
			this.resultContainer.y = tblHeaderBg.y + 59;
			this.resultContainer.x = 67;
			...
			$.post(links.getDetails, {round: details.round, tableId: window.tableNum}, (response) => {
				...
				let winnerTextObj = new createjs.Text(winnerText, 'bold 30px lato-regular', '#fff');
				winnerTextObj.x = winnerCircle[0].x;
				winnerTextObj.y = winnerCircle[0].y - 19.3;
				...
				if (parityText) {
					let parityTextObj = new createjs.Text(parityText, 'bold 30px lato-regular', '#fff');
					parityTextObj.x = winnerCircle[1].x;
					parityTextObj.y = winnerCircle[1].y - 19.3;
					parityTextObj.textAlign = 'center';
					this.betDetails.addChild(parityTextObj);

					let sizeTextObj = new createjs.Text(sizeText, 'bold 30px lato-regular', '#fff');
					sizeTextObj.x = winnerCircle[2].x;
					sizeTextObj.y = winnerCircle[2].y - 19.3;
				...
				let mdHeight = (betCount + 1) >= 8 ? 800 : (betCount + 1) * (40 * 1.2) + 410;
				...
				let tblTotalText = new createjs.Text(window.language.menu.total, 'bold 20px lato-regular', '#231f20');
			    ...
			    for (var j = 0; j < betCount; j++) {
					...
					if (isText) {
						...
						betTextObj[j].y = betCircle[j].y - 12.5;
						...
						betTextObj[j].x = betCircle[j].x - 11.5;
						betTextObj[j].y = betCircle[j].y - 10;
						this.tblDataContainer.addChild(betTextObj[j]);
					}

					betAmtObj[j] = new createjs.Text(this.formatNumber(this.betHistory[j].bet_money), 'bold 19px lato-regular', '#231f20');
					betAmtObj[j].x = tblHeaderBg.x + 370;
					betAmtObj[j].y = (tblHeaderBg.y + 46.5) + (j * 40);
					...
					winLoseObj[j] = new createjs.Text(this.formatNumber(this.betHistory[j].win_money), 'bold 19px lato-regular', winLoseColor);
					winLoseObj[j].x = tblHeaderBg.x + 570;
					winLoseObj[j].y = (tblHeaderBg.y + 46.5) + (j * 40);
				...
				let totalBetObj = new createjs.Text(this.formatNumber(totalBetAmt), 'bold 20px lato-regular', '#242021');
				totalBetObj.x = tblHeaderBg.x + 370;
				totalBetObj.y = (tblHeaderBg.y + tableHeight) + 6.5;
				...
				let totalWinLoseObj = new createjs.Text(this.formatNumber(totalWinLoseAmt), 'bold 20px lato-regular', winLoseColor);
				totalWinLoseObj.x = tblHeaderBg.x + 570;
				totalWinLoseObj.y = (tblHeaderBg.y + tableHeight) + 6.5;
				...
				let resultValueLeft = new createjs.Text(valDragon.value, 'bold 33px lato-regular', '#fff');
				resultValueLeft.x = leftResultCircle.x;
				resultValueLeft.y = leftResultCircle.y - 22;
				resultValueLeft.textAlign = 'center';
				this.cardsContainer.addChild(resultValueLeft);

				let valTiger = cardValue(this.gameInfo.tiger)
				let resultValueRight = new createjs.Text(valTiger.value, 'bold 33px lato-regular', '#fff');
				resultValueRight.x = rightResultCircle.x;
				resultValueRight.y = rightResultCircle.y - 22;
				...
				let scrollprop = this.scrollable(this.betDetails, this.resultContainer, 766, 505);
