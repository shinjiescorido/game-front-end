...
export default() => {
	instance = instance || new blu.Component({
		main() {
			...
			let roadMapW = 550;
      		let cellW = roadMapW / 23; //temp size
			let roadMapH = 150;
			let cellH = roadMapH / 6;

			let beadRoadMapColumns = 9; let roadMapColumns = 14;
			...
			this.pred_b_bg.graphics.beginStroke("#999").beginFill("#e6e6e6").drawRect(0, 0, cellW * 1.5, roadMapH);
			this.pred_r_bg = new createjs.Shape;
			this.pred_r_bg.graphics.beginStroke("#999").beginFill("#e6e6e6").drawRect(cellW * 1.5, 0, cellW * 1.5, roadMapH);

			//=== drawRoadMap(columns, rows, cell width, cell height, roadMap_container )
			...
			let prediction_icons = new createjs.SpriteSheet({
				...
				animations: {
					"b": 0, "r": 1, "bhc": 2, "rhc": 3, "bfc": 4, "rfc": 5, "bslash": 6, "rslash": 7
				}
			});
			let pred_W = cellW * 1.5;
			let pred_H = cellH * 1.5;
			let pred_padding = 6;
			let pred_scaleXY = (pred_W - (pred_padding * 2)) / this.context.getResources("prediction").height;

			let pred_b = new createjs.Sprite(prediction_icons, "b");
				pred_b.scaleX = pred_b.scaleY = pred_scaleXY;
				pred_b.x = 6; pred_b.y = 6;

			let pred_r = new createjs.Sprite(prediction_icons, "r");
				pred_r.scaleX = pred_r.scaleY = pred_scaleXY;
				pred_r.x = pred_W + 6; pred_r.y = 6;

			let pred_bhc = new createjs.Sprite(prediction_icons, "bhc");
				pred_bhc.scaleX = pred_bhc.scaleY = pred_scaleXY;
				pred_bhc.x = 6; pred_bhc.y = pred_H + 6;

			let pred_rhc = new createjs.Sprite(prediction_icons, "rhc");
				pred_rhc.scaleX = pred_rhc.scaleY = pred_scaleXY;
				pred_rhc.x = pred_W + 6; pred_rhc.y = pred_H + 6;

			let pred_bfc = new createjs.Sprite(prediction_icons, "bfc");
				pred_bfc.scaleX = pred_bfc.scaleY = pred_scaleXY;
				pred_bfc.x = 6; pred_bfc.y = (pred_H * 2) + 6

			let pred_rfc = new createjs.Sprite(prediction_icons, "rfc");
				pred_rfc.scaleX = pred_rfc.scaleY = pred_scaleXY;
				pred_rfc.x = pred_W + 6; pred_rfc.y = (pred_H * 2) + 6;

			let pred_bslash = new createjs.Sprite(prediction_icons, "bslash");
				pred_bslash.scaleX = pred_bslash.scaleY = pred_scaleXY;
				pred_bslash.x = 6; pred_bslash.y = (pred_H * 3) + 6;

			let pred_rslash = new createjs.Sprite(prediction_icons, "rslash");
				pred_rslash.scaleX = pred_rslash.scaleY = pred_scaleXY;
				pred_rslash.x = pred_W + 6; pred_rslash.y = (pred_H * 3) + 6;

      		this.prediction_con.addChild(this.pred_b_bg, this.pred_r_bg);
			this.prediction_con.addChild(pred_b, pred_r, pred_bhc, pred_rhc, pred_bfc, pred_rfc, pred_bslash, pred_rslash);
			...
			let result = [
				...
				winner: "dragon",	winnerTotal: "8",	winnerOddEven: "even",	dragonBigSmall: "big",	tigerBigSmall: "small"	},
				{ // 5
				winner: "dragon",	winnerTotal: "8",	winnerOddEven: "even",	dragonBigSmall: "big",	tigerBigSmall: "big"	},
				{ // 4
				winner: "dragon",	winnerTotal: "8",	winnerOddEven: "even",	dragonBigSmall: "big",	tigerBigSmall: "big"	},
				{ // 5
				winner: "tiger",	winnerTotal: "8",	winnerOddEven: "even",	dragonBigSmall: "big",	tigerBigSmall: "big"	},
			...
			let toggleView = false;
      		...
      		this.roadMap_bg.addEventListener("click", (e) => {
				if(!toggleView) {
					toggleView = true;
					...
					this.removeChild(this.bigRoad_grid, this.bigeyeRoad_grid, this.smallRoad_grid, this.cockroachRoad_grid);
					this.bigRoad_grid = this.drawRoadMap(15, 6, cellW / 2, cellH / 2, cellW * beadRoadMapColumns, 0, this.roadMap_container["big_road"]);
					this.bigeyeRoad_grid = this.drawRoadMap(15, 3, cellW / 2, cellH / 2, cellW * beadRoadMapColumns, cellH * 3, this.roadMap_container["big_eye_road"]);
					this.smallRoad_grid = this.drawRoadMap(8, 3, cellW / 2, cellH / 2, cellW * beadRoadMapColumns, cellH * 4.5, this.roadMap_container["small_road"]);
					this.cockroachRoad_grid = this.drawRoadMap(7, 3, cellW / 2, cellH / 2, cellW * (beadRoadMapColumns + 4), cellH * 4.5, this.roadMap_container["cockroach_road"]);

					this.addChild(this.bigRoad_grid, this.bigeyeRoad_grid, this.smallRoad_grid, this.cockroachRoad_grid);

					this.shiftRoadMapIcons(cellW * 3, "cockroach_road");

					this.roadMap_bg.graphics.clear().beginFill("#fff").drawRect(0,0,395, 150);
				} //end if 
				else {
					this.roadMap_bg.graphics.clear().beginFill("#fff").drawRect(0,0,550, 150);
					toggleView = false;
					data = this.dataForRoadmap(result, "bead_road");
					this.clearRoadMapIcons("bead_road");
					this.writeOnRoadMap(data, cellW, cellH, this.context.getResources("bead_road_dragon").height, "bead_road");
