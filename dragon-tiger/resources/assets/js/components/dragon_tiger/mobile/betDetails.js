...
export default() => {
	instance = instance || new blu.Component({
		main() {
			...
			this.bg.graphics.beginFill(this.context.theme_color[window.theme].base_color).drawRoundRectComplex(0, 0, 200, 110, 0, 10, 10, 0);
			this.bg.cache(0, 0, 200, 110);
			this.bg.alpha = 0.8;
			this.addChild(this.bg);

			this.user_money = new createjs.Text("0", "26px bebas-regular", this.context.theme_color[window.theme].menu_text_color);
			this.user_money.textAlign = "left";
			this.user_money.x = 170;
			...
			this.total_bet_label = new createjs.Text(window.language.menu.totalbetcaps, window.language.locale == "zh" ? "15px arial" : "14px arial", this.context.theme_color[window.theme].text_color);
			this.total_bet_label.textAlign = "center";
			this.total_bet_label.y = 80;
			this.total_bet_label.x = 40;
			this.addChild(this.total_bet_label);

			this.winning_label = new createjs.Text(window.language.bet_details.winningscaps, window.language.locale == "zh" ? "15px arial" : "14px arial", this.context.theme_color[window.theme].text_color);
			this.winning_label.textAlign = "center";
			this.winning_label.y = 80;
			this.winning_label.x = 140;
			this.addChild(this.winning_label);

			if (window.theme == 'white') {
				this.total_bet_label.font = window.language.locale == "zh" ? "bold 15px arial" : "bold 14px arial";
				this.winning_label.font = window.language.locale == "zh" ? "bold 15px arial" : "bold 14px arial";
			}
			else {
				this.total_bet_label.font = window.language.locale == "zh" ? "15px arial" : "14px arial";
				this.winning_label.font = window.language.locale == "zh" ? "15px arial" : "14px arial";
			}
		},
		setUserMoney(money) {
			let userMoney = 0;
		...
		changeTheme(new_theme) {
			...
			this.bg.graphics.clear().beginFill(this.context.theme_color[new_theme].base_color).drawRoundRectComplex(0, 0, 200, 110, 0, 10, 10, 0);
			this.bg.cache(0, 0, 200, 110);
		...
		reinit(anim) {
			...
			if(this.context.context.user_money != 0) {
				...
				if(this.context.context.user_money.toString().length > 8) {
					this.user_money.font = "26px bebas-regular";
				}
			...
			// }
	      	...
	      	if(this.user_money.bounds.width > 120) {
		        ...
		        this.user_money.con.mask.graphics.clear().beginFill("#fff").drawRect(-5, 0, 125, this.user_money.bounds.height + 10);
