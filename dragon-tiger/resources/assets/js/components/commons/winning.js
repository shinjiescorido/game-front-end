...
export default(win_assets) => {
	instance = instance || new blu.Component({
		...
		isWin () {
			...
			let toPos2 = this.circle(300,8,0,0);

			let toPos3 = this.circle(280,8,0,0);
			let toPos4 = this.circle(350,8,0,0);

			let toPos5 = this.circle(250,8,0,0);
		...
		createWinText(text, color) {
			...
			if(window.language.locale == "jp" || window.language.locale == "en" ) {
				fontcon = false;
			}

			for(var x = 0;x<textArray.length;x++) {
				this.win_text[x] = new createjs.Text(textArray[x].toUpperCase(), fontFormat(65, "black", 'arvo' , fontcon), color);
