...
export default() => {
	...
	instance = instance || new blu.Component({
		main() {
			...
			let textToDisplay = [window.language.menu.soundeffects,window.language.menu.voice, window.language2.com_sub_settings_darktheme, window.language.menu.showtutorial]
      	...
      	if (window.nonInstall) {
			...
			if (window.nonInstall) {
				this.volumeCircle.visible = false;

				let initCoorX = window.config.volume * 170;
				$('#volumeCircle').css('left', initCoorX + 840);

				if (parseInt(window.vendorSound)) {
					this.makeDraggable('volumeCircle');
				}
			}
			else {
      ...
      this.volumeMax.addEventListener("mousedown", (e) => {
      	...
      	this.volumeCircle.x = this.volumeCurrent.x + newVolume;
				...
				if (window.nonInstall) {
					this.context.playerVid.volume = 1;
					$('#volumeCircle').css('left', 1012);
				}
      });

      if (!parseInt(window.vendorSound)) {
				this.volumeCurrent.graphics.clear().beginFill("#4c4c4c").drawRoundRect(0, 0, this.pos > 0 ? this.pos-80 : 0 , 5, 2);
				this.volumeCircle.graphics.clear().beginFill("#4c4c4c").drawCircle(0, 0, 16);
				$('#volumeCircle').css({background: '#4c4c4c'});
			...
			for (var i = 0; i < lblSettingsArr.length; i++) {
				...
				this.enabledX = this.settingsCircle[i].x;
		...
		enableSetting(i) {
			if(this.settingsToggleHit[i].status == "enabled") return;

			createjs.Tween.get(this.settingsCircle[i], { loop: false })
				  .to({ x: this.enabledX }, 0)

			this.settingsToggle[i].graphics.clear().beginFill("#fff").drawRoundRect(0, 0, 65, 25, 14);
			this.settingsCircle[i].graphics.clear().beginFill("#2e7d32").drawCircle(0, 0, 10);
			this.settingsToggleHit[i].status = "enabled";

			if(this.settingsToggleHit[i].name.toLowerCase() == "voice") {
			...
			if(this.settingsToggleHit[i].name.toLowerCase() == "sound effects") {
				$.post(route, {effect : 1}, (response) => {
					config.effect = '0';
				});
			}

			if(this.settingsToggleHit[i].name == "Dark Theme") {
				window.theme = "black";
				this.context.component_ingameTutorial.changeTheme(window.theme);
				$.post(route, {screen : 0}, (response) => {

				});
			}

			if (this.settingsToggleHit[i].name == "Show Tutorial") {
		...
		makeDraggable(element) {
			...
			let currCoor = src.style.left.slice(0, src.style.left.length-2);
			let clientX;
			let clientY;
			let offsetX;
			let offsetY;
			...
			src.addEventListener('touchend', (e) => {
  			...
  			// Update coordinate
				currCoor = src.style.left.slice(0, src.style.left.length-2);

				if (this.context.playerVid.volume > 0.1) {
					$.post(route, {volume : this.currVol}, (response) => { });
					$.post(route, {voice : 1}, (response) => { });
					$.post(route, {effect : 1}, (response) => { config.effect = '1'; });
				}
				else {
					this.context.playerVid.volume = 0;
					$.post(route, {volume : 0}, (response) => { });
					$.post(route, {voice : 0}, (response) => { });
					$.post(route, {effect : 0}, (response) => { config.effect = '0'; });
				}
			}, false);

			src.addEventListener('touchmove', (e) => {
				let newCoor = parseInt(currCoor) + (e.changedTouches[0].pageY - clientY);
				if (window.matchMedia("(orientation: landscape)").matches) {
					newCoor = parseInt(currCoor) + (e.changedTouches[0].pageX - clientX);
				}

				if (newCoor < 845 || newCoor > 1012) return;

			  src.style.left = `${newCoor}px`;

  			this.setVolume(src, newCoor);
			}, false);
		},
		setVolume(src, newCoor) {
			this.currVol = Math.round( ((newCoor - 840) / 170) * 10 ) / 10;
			this.volumeCurrent.graphics.clear().beginFill("#7cb342").drawRoundRect(0, 0, this.default_vol * parseFloat(this.currVol), 5, 2);

			this.context.playerVid.volume = this.currVol;
		...
		disableSound() {
			if (this.settingsToggleHit[0].status == "disabled") return;
		...
		enableSound() {
			if(this.settingsToggleHit[0].status == "enabled") return;
