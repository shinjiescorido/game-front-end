      ...
      'PTS': [ '1', '5', '10', '50', '100', '500', '1000', '5000', '10000', '20000','50000', 'max' ],
		...
		makeChips(){
			...
			for (var i = 0; i < 16; i++) {
				...
				let sprite_instance = createSprite(this.context.getResources(chip_name), 72, 72, chips[i]);
        ...
        this.context.chipsConf.forEach((c) => {
          if(chips[x].chip == c.chipval) {
            chip_name = "chip_"+c.chipName.split("_")[2]
				...
				instanceMask.graphics.beginFill('#e1e9ff').drawCircle(0, 0, maskRadius);
				...
				if (instanceTxt.text.toString().length > 4) {
					instanceTxt.font = 'normal 18px bebas-regular'
				}
		...
		removeBetAreaChips (table,index) {
			...
			if (table.bet_amt_text) {
				table.bet_amt_text.text = 0;
			}
