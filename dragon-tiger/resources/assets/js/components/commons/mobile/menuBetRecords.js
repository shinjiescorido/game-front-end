	...
	instance = instance || new blu.Component({
		main() {
			...
			if(window.language.locale == "zh") {
				this._headerTxt.y = this._modalHeader.y - 4;
			} else {
		...
		initRecords(type) {
			...
			switch(type) {
				...
				case ('betlogs'):
					$.post(links.getBetLogs, {tableId: window.tableNum, mobile: true}, (response) => {
		...
		displayBetLogs() {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				// betStartingCred[i] = new createjs.Text(this.formatNumber(data[0].user_money), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
        ...
        if(this._betLogs.data[i].status && this._betLogs.data[i].status.toLowerCase() === 'w') {
          ...
          voidText.x = voidShape.getBounds().width - 270;
		...
		displayGameHistory() {
			...
			if (this._gameHistory.data.length == 0) {
				let noDataText = new createjs.Text('NO DATA TO DISPLAY', 'bold 30px lato-regular', this.context.theme_color[window.theme].labelcolor);
				noDataText.x = this._modalWidth / 2;
				noDataText.y = 300;
				noDataText.textAlign = 'center';
