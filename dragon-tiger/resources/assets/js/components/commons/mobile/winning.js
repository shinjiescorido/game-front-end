...
export default(win_assets) => {
	instance = instance || new blu.Component({
		...
		main() {
			...
			this.gold_instance.graphics.beginLinearGradientFill(["transparent","#d8bd69","transparent"], [0, 0.5, 1], 0, 0, this.context.stage.baseWidth-180, 0).drawRect(0, 0, this.context.stage.baseWidth-180, 90);
		...
		createWinText(text, color) {
			...
			for(var x = 0;x<textArray.length;x++) {
				this.win_text[x] = new createjs.Text(textArray[x].toUpperCase(),window.language.locale == "zh"? "400 80px arvo-regular" : "700 80px arvo-regular", color);
				...
				// if(textArray[x] == "i" || textArray[x] == "t") {
				// 	posX += 30
				// }
				// if(space && x>space) {
				// 	posX -= 30;
