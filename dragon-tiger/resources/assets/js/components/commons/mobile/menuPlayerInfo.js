...
export default() => {
	instance = instance || new blu.Component({
      ...
      this.scaleX = this.scaleY = 1.5;
			...
			// let userMoneyAmt = 0;
			// if (window.casino == 'SS') {
			// 	userMoneyAmt = this.context.component_winAmount.numberWithCommas(parseFloat(this.context.context.user_money).toFixed(2));
			// }
			// else {
			// 	userMoneyAmt = this.context.component_winAmount.numberWithCommas(parseInt(this.context.context.user_money));
			// }
			...
			this.channel = new createjs.Text("Dragon Tiger "+window.tableNum,"13px arial", this.context.theme_color[window.theme].modal_txt_color);
		...
		updatePlayerInfo(balance, round, dealer) {
			this.balance.text = moneyFormat(balance);
