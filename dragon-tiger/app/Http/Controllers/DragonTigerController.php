...
class DragonTigerController extends Controller
{
    ...
    public function init($table, $gameType = null)
    {
        ...
        //     'title' => 'test',
        //     'betRange' => '5-100',
        //     'roomPass' => '52a6eb687cd22e80d3342eac6fcc7f2e19209e8f83eb9b82e81c6f3e6f30743b',
        //     'bankerUsername' => 'demo',
        //     'vendorId' => 10,
        //     'bankerMultiplier' => '1.0',
        //     'vendorName' => 'mike',
        //     'token' => 'cde6319f-9cc6-11e8-a064-00155d063d02',
        //     'isJunket' => '2'
        //   ];
        ...
        if(Auth::user()->is_junket > 0) {
            ...
            //room create
            if(strtolower(env('CUR_GAME')) === 'dragon-tiger') {
                $currentTable = strtolower('dragontiger-'.$table);
            }
    ...
    public function getJunketUsers($roomId) {
       return DB::select('CALL dragontiger.USP_JUNKET_PAYOUT('. $roomId .')');
    }
    ...
    public function setRoomInfo(Request $request)
    {
        ...
        DB::select('CALL nihtan_api.USP_ROOMS_SAVE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($flag, 'dragontiger', $roomId, $tableId, $roomType, $userId, $status, '', $password, '', 0, 0));
        return $password;
    }
    ...
    public function getVendor($token, $table, $range, $gameType)
    {
      $roomData;
      if ($range == null) {
        $roomData = DB::connection('dragontiger')->table("dragontiger.rooms")
    ...
    public function getBetLogs(Request $request) {
        ...
        if ($mobile) {
            ...
            return DB::select('CALL dragontiger.USP_NEW_GET_BET_HISTORY(?, ?, ?, ?, ?)', array($gameType, $userId, $tableId, $roundNum, $page));
        }
    }
    ...
    public function getJunketLogs(Request $request)
    {
        ...
        return DB::select('CALL dragontiger.USP_GET_JUNKET_BET_HISTORY(?, ?, ?, ?, ?, ?)', array($gameType, (int)$roomId, (int)$userId, (int)$tableId, (int)$roundNum, (int)$page));
    }
        ...
        if($check ===  true) {
            try {
                ...
                $insertQry = "INSERT INTO bets"
                    . " (`bet_history`, `created_at`,`round_id`, `type`, `user_id`, `room_id`, `total_bet`, "
                    . " `total_rolling`, `total_winning`, `total_win`, `total_lost`, `bet_range`, `device`, `bet_id`, `session_id`, `currency`, `play_type`, `multiplier`)"
