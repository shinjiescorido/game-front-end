...
export default() => {
	instance = instance || new blu.Component({
		...
		main() {
			...
			this.background.graphics.beginFill(this.context.theme_color[window.theme].base_color).drawRect(-28,-20,217,190);
			this.background.cache(-30,-20,220,190);
			this.addChild(this.background);

			this.user_avatar = new createjs.Bitmap(this.context.getResources(window.config.default));
			// this.user_avatar = new createjs.Bitmap("/img/avatar/poker/red_queen.png");
			this.user_avatar.scaleX = this.user_avatar.scaleY = 0.8;
      ...
      if(textbounds.width > 80) {
        ...
        this.addChild(this.user_name);
			...
			this.bar_blue.cache(0,0,170,20)
			this.bar_blue.y = 100;
			this.bar_blue.x = -5;
		...
		changeTheme(new_theme) {
			this.background.graphics.clear().beginFill(this.context.theme_color[new_theme].base_color).drawRect(-28,-20,217,190);
