	...
	instance = instance || new blu.Component({
		...
		initRecords(type) {
			...
			switch(type) {
				...
				case ('betlogs'):
					...
					$.post(links.getBetLogs, {tableId: window.tableNum, bet_type}, (response) => {
		...
		displayBetLogs() {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				// betStartingCred[i] = new createjs.Text(this.formatNumber(startingCred), 'bold 11px lato-regular', this.context.theme_color[window.theme].labelcolor);
		...
		paginate(type, pageNum, container, navigation, callback) {
			let pageUrl = '';
			//Live
			let baseUrl = window.poker_domain+window.tableNum+'/'+range; // window.location.href
		  ...
		  $.get(pageUrl, {tableId: window.tableNum, bet_type}, (response) => {
