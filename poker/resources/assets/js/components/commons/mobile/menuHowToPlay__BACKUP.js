...
export default() => {
	...
	instance = instance || new blu.Component({
      ...
      this.x = this.context.stage.baseWidth - this._modalWidth - 140;
			this.y = 90;
			this.visible = false;
			...
			this._modalHeader.graphics.beginLinearGradientFill([this.context.theme_color[window.theme].base_color, this.context.theme_color[window.theme].gradColor2], [.1, .9], 10, 10, 10, 52)
        ...
        .drawRoundRect(0, 0, this._modalWidth, 52, 3);
			...
			//Header Text
			this._headerTxt = new createjs.Text('HOW TO PLAY',"bold 24px lato-regular", this.context.theme_color[window.theme].labelcolor);
			this._headerTxt.x = this._modalHeader.x + 10;
			this._headerTxt.y = this._modalHeader.y + 9;
			this.addChild(this._headerTxt);

			//Header Close button
			this._headerClose = new createjs.Text("X","bold 24px lato-regular", this.context.theme_color[window.theme].labelcolor);
			this._headerClose.x = this._modalWidth - 50;
      ...
      let howToTabPage = [];
			...
			for (var i = 0; i < tabLbl.length; i++) {
				...
				howToTab[i].graphics.beginFill(this.context.theme_color[window.theme].bet_record_tab).drawRect(0, 0, this._modalWidth / 2, 80);
				howToTab[i].x = (this._modalWidth / 2) * i;
				howToTab[i].y = this._modalHeader.y + 52;
        ...
        let tab_width = this._modalWidth / 2;
				...
				howToTabIcon[i].x = (tab_width * i) + tab_width/2;
				howToTabIcon[i].y = howToTab[i].y + 8;

				howToTabLbl[i] = new createjs.Text(tabLbl[i],"bold 22px lato-regular", "#2b2b2b");
				howToTabLbl[i].x = (tab_width * i) + tab_width/2;
				howToTabLbl[i].y = howToTab[i].y + 42;
				...
				((i) => {
			       	howToTab[i].addEventListener("mousedown", (e) => {
				        	for (var x = 0; x < tabLbl.length; x++) {
      								...
      								howToTab[x].graphics.clear().beginFill(this.context.theme_color[window.theme].bet_record_tab).drawRect(0, 0, this._modalWidth / tabLbl.length, 80);
		...
		pageGameRules() {
      let font_t = "26px lato-black";
      let font_b = "24px LatoRegular";
      let font_h = "bold 24px LatoRegular";
      let font_s = "22px LatoRegular";
      let lineheight_b = 26;
      let linewidth_b = this._modalWidth - 60;
      let color_t = "#ffa000";
      let color_b = "#eee";
      let color_s = "#fff";
      let color_h = "#000";
      ...
      image.sa.x = image.sb.x = linewidth_b * 0.55;
      ...
      tablehead_a.bg.graphics.beginFill("#b4b4b4").drawRect(-1, 0, 702, 30);
      tablehead_a.bg.x = 20;
      tablehead_a.a.x = tablehead_a.bg.x + 85;
      tablehead_a.b.x = tablehead_a.bg.x + 255;
      tablehead_a.c.x = tablehead_a.bg.x + 520;

      let table_a = this.drawTable([170, 170, 360], 5, 450);
      ...
      tablehead_b.bg.graphics.beginFill("#b4b4b4").drawRect(-1, 0, 702, 30);
      tablehead_b.bg.x = 20;
      tablehead_b.a.x = tablehead_b.bg.x + 175;
      tablehead_b.b.x = tablehead_b.bg.x + 525;
      let table_b = this.drawTable([350, 350], 7, 210);
      ...
      con.setBounds(0, 0, this._modalWidth - 40, con.getBounds().height + 300);
      ...
      scrollprop.toTop_btn.scaleX = scrollprop.toTop_btn.scaleY = 2.5;
      scrollprop.toTop_btn.x -= 25;
      ...
      Object.values(image).forEach((o, i) => {
        if(o.image.width > linewidth_b - 10) {
          o.scaleX = o.scaleY = (linewidth_b - 10) / o.image.width;
        }
        if([3, 8, 9, 10].includes(i)) {
      ...
      title.b.y = body.a.y + body.a.height + 140;
      image.b.y = title.b.y;
      ...
      body.g.y = title.g.y + title.g.height + 20;
        ...
        title.a.x = title.b.x = title.d.x = title.e.x = title.f.x = title.g.x = linewidth_b * 0.5 + 10;
        body.a.x = body.b.x = body.d.x = body.e.x = body.f.x = body.g.x = linewidth_b * 0.5 + 10;
        body.a.lineWidth = body.b.lineWidth = body.d.lineWidth = body.e.lineWidth = body.f.lineWidth = body.g.lineWidth = linewidth_b * 0.4;
      ...
      con.setBounds(0, 0, this._modalWidth - 40, con.getBounds().height + 100);
    ...
    scrollable(page, content) {
      ...
      view.graphics.drawRect(0, 0, this._modalWidth - 10, this._modalHeight - 120); //.beginStroke("#fff").setStrokeStyle(0.1)
      view.setBounds(0, 0, this._modalWidth - 10, this._modalHeight - 120);
      view.x = content.x;
      view.y = content.y;
      view.hitArea = new createjs.Shape();
      view.hitArea.graphics.beginFill("#fff").drawRect(0, 0, this._modalWidth - 10, this._modalHeight - 120);
      page.addChild(view);
      content.mask = view;
      ...
      if(content_bounds.height > view_bounds.height) {
        ...
        top_glow.graphics.beginLinearGradientFill(["#ff9a28", "transparent"], [0, 1], 0, -10, 0, 40).drawRect(0, 0, view_bounds.width + 10, 50);
        top_glow.x = 0;
        top_glow.y = top - 10;
        top_glow.alpha = 0.7;
        ...
        view.addEventListener("pressup", (e) => {
          curDown = false;
        });
        view.addEventListener("pressmove", (e) => {
    ...
    makeScrollToButton(content, text, from_Y, to_Y, scrollprop) {
      ...
      link_bg.graphics.beginFill("#ffa000").drawRect(0, 0, this._modalWidth - 27.5, 40);
      link_bg.alpha = 0;
      ...
      link_bg.x = 0;
      link_bg.y = from_Y;
      let link_lbl = new createjs.Text(text, "bold 24px lato-black", "#ffa000");
