	...
	instance = instance || new blu.Component({
		...
		startTime(time, pause, is_hold, totalTime) {
			...
			this.timer_interval.push( setInterval( () => {
				...
				if(!is_hold) {
	 				this.setTime(this.timer-1, pause, totalTime);
				}
				if(!pause){
					this.timer--;
				}
		...
		gameSpecificHandle () {
			switch(this.context.roundphase) {
				case "startround":
					this.context.component_tableDraw.stopBetAreaAnimation(this.context.component_betBoard.bet_areas[0]);
					this.context.component_tableDraw.stopBetAreaAnimation(this.context.component_betBoard.bet_areas[1]);
					this.context.component_tableDraw.stopBetAreaAnimation(this.context.component_betBoard.bet_areas[2] || null);
					break;
				case "flop" :
					if(this.context.component_extraBetButtons.fold_check_btn.has_fold) return;
					if(!parseInt(this.context.component_betBoard.bet_areas[0].total_bet_amt)) return;

					this.context.component_extraBetButtons.hideButtons();
					if(!this.context.component_tableDraw.flop_area.total_bet_amt)  {
						this.context.component_extraBetButtons.fold_check_btn.has_fold = true
						this.context.component_extraBetButtons.setFoldCheck(this.context.component_tableDraw.flop_area,"fold");
							this.context.component_extraBetButtons.fold_check_btn.has_fold = true;
					}
					...
					break;
				case "turn" :
					if(this.context.component_extraBetButtons.fold_check_btn.has_fold && !this.context.component_tableDraw.flop_area.total_bet_amt) return;
					if(this.context.component_extraBetButtons.fold_check_btn.has_check_turn) return;
					this.context.component_extraBetButtons.hideButtons();
					if(this.context.component_betBoard.bet_areas[0].total_bet_amt) {
						if(!this.context.component_tableDraw.turn_area.total_bet_amt)  {
							this.context.component_extraBetButtons.setFoldCheck(this.context.component_tableDraw.turn_area,"check");
							this.context.component_extraBetButtons.fold_check_btn.has_check_turn = true;
						}
					}
				...
				case "river" :
					if(this.context.component_extraBetButtons.fold_check_btn.has_fold && !this.context.component_tableDraw.flop_area.total_bet_amt) return;
					if(this.context.component_extraBetButtons.fold_check_btn.has_check_river) return;
					this.context.component_extraBetButtons.hideButtons();
					if(this.context.component_betBoard.bet_areas[0].total_bet_amt) {
						if(!this.context.component_tableDraw.river_area.total_bet_amt)  {
							this.context.component_extraBetButtons.setFoldCheck(this.context.component_tableDraw.river_area,"check");
							this.context.component_extraBetButtons.fold_check_btn.has_check_river = true;
						}
    				}
					this.context.component_tableDraw.stopBetAreaAnimation(this.context.component_tableDraw.river_area);
        			this.context.toggleView("result");
					break;
			}
		},
		cloneTimer(){
