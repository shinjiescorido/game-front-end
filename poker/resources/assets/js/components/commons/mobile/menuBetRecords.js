...
export default(links, bet_type) => {
	instance = instance || new blu.Component({
		main() {
			...
			//Modal dimensions
			this._modalWidth = 1000;
			this._modalHeight = 520;

			this.y = 106;
			this.x = (this.context.stage.baseWidth / 2) - (this._modalWidth / 2) - 90;
			...
			this._modalHeader.graphics.beginLinearGradientFill([this.context.theme_color[window.theme].base_color, this.context.theme_color[window.theme].gradColor2], [.1, .9], 14, 14, 14, 49)
				.drawRoundRect(0, 0, this._modalWidth, 49, 3);
			this._modalHeader.regY = 14;
			this.addChild(this._modalHeader);

			//Header Text
			this._headerTxt = new createjs.Text(window.language.menu.recordscaps,"bold 15px arial", this.context.theme_color[window.theme].labelcolor);
			this._headerTxt.x = this._modalHeader.x + 14;
			this._headerTxt.y = this._modalHeader.y - 2;
			this._headerTxt.scaleX = this._headerTxt.scaleY = 1.4;
			...
			this._headerClose.x = this._modalWidth - 41;
			this._headerClose.y = this._modalHeader.y - 2;
			this._headerClose.scaleX = this._headerClose.scaleY = 1.4;
			...
			this._headerCloseHit.y = this._headerClose.y - 2;
			...
			//Init bet records tabs
			for (var i = 0; i < this.betRecordsLbl.length; i++) {
				this.betRecordsTab[i] = new createjs.Shape();
				this.betRecordsTab[i].graphics.beginFill(this.context.theme_color[window.theme].bet_record_tab).drawRect(0, 0, this._modalWidth / 3, 60);
				this.betRecordsTab[i].x = (this._modalWidth / 3) * i;
				this.betRecordsTab[i].y = this._modalBg.y;
				...
				this.betRecordstabIcon[i] = new createjs.Sprite(icon_spriteSheet,"normal");
				this.betRecordstabIcon[i].x = (((this._modalWidth / 3) * i) + ((this._modalWidth / 3) / 2)) - 10;
				this.betRecordstabIcon[i].y = this._modalBg.y + 8;

				this.betRecordstabLbl[i] = new createjs.Text(this.betRecordsLbl[i],window.language.locale == "zh" ? "bold 20px arial" : "bold 13px arial", "#2b2b2b");
				this.betRecordstabLbl[i].x = ((this._modalWidth / 3) * i) + ((this._modalWidth / 3) / 2);

				if(window.language.locale == "zh") {
					this.betRecordstabLbl[i].y = this._modalBg.y + 30;
				} else {
					this.betRecordstabLbl[i].y = this._modalBg.y + 35;
				}

				this.betRecordstabLbl[i].textAlign = "center";

				if (i == 0) {
					this.betRecordsTab[i].graphics.clear().beginFill(this.context.theme_color[window.theme].bet_record_tab_act);
					this.betRecordstabIcon[i].gotoAndPlay("active");
					this.betRecordstabLbl[i].color = "#ff9b28";
				}

				this.addChild(this.betRecordsTab[i], this.betRecordstabIcon[i], this.betRecordstabLbl[i]);

				//Tab click event
				((i) => {
				  this.betRecordsTab[i].addEventListener("mousedown", (e) => {
						for (var x = 0; x < this.betRecordsLbl.length; x++) {
							this.betRecordsTab[x].graphics.clear().beginFill(this.context.theme_color[window.theme].bet_record_tab).drawRect(0, 0, this._modalWidth / this.betRecordsLbl.length, 60);
							this.betRecordstabIcon[x].gotoAndPlay("normal");
							this.betRecordstabLbl[x].color = "#2b2b2b";
						}
			...
			this._tblHeader.y = this._modalBg.y + 80;
		...
		changeTheme(new_theme) {
			this._modalHeader.graphics.clear().beginLinearGradientFill([this.context.theme_color[new_theme].base_color, this.context.theme_color[new_theme].gradColor2], [.1, .9], 14, 14, 14, 49)
				.drawRoundRect(0, 0, this._modalWidth, 49, 3);
			...
			let betRecordsIcon = ["transfer-"+new_theme, "bet-logs-"+new_theme, "game-history-"+new_theme];

			for (var i = 0; i < this.betRecordsLbl.length; i++) {
				this.betRecordsTab[i].graphics.clear().beginFill(this.context.theme_color[new_theme].bet_record_tab).drawRect(0, 0, this._modalWidth / 3, 60);
				this.betRecordsTab[i].name = betRecordsIcon[i];
				...
				let icon_spriteSheet = new createjs.SpriteSheet(icon_spritesheet_data);
				this.betRecordstabIcon[i].spriteSheet = icon_spriteSheet;
				this.betRecordstabIcon[i].gotoAndPlay("normal");
			}
		},
		initRecords(type) {
			//Remove all
			...
			switch(type) {
				case ('transferlogs'):
					...
					this.betRecordsTab[0].graphics.clear().beginFill(this.context.theme_color[window.theme].bet_record_tab_act);
					this.betRecordstabIcon[0].gotoAndPlay("active");
					this.betRecordstabLbl[0].color = "#ff9b28";
				...
				case ('betlogs'):
					$.post(links.getBetLogsMobile, {tableId: window.tableNum, bet_type: bet_type, mobile: true}, (response) => {
		...
		displayTransferLogs() {
			...
			for (var i = 0; i < transferTblHeader.length; i++) {
				transferHeader[i] = new createjs.Text(transferTblHeader[i], window.language.locale == "zh" ? 'bold 18px lato-regular' : 'bold 13px lato-regular', '#2b2b2b');
				transferHeader[i].x = (128 * i) + 70;

				if(window.language.locale == "zh") {
					transferHeader[i].y = this._tblHeader.y + 3;
				} else {
				...
				if (i != transferTblHeader.length - 1) {
					...
					tableBorder[i].graphics.setStrokeStyle(1).beginStroke(this.context.theme_color[window.theme].tbl_border)
						.moveTo((130 * i) + 130, this._tblHeader.y).lineTo((130 * i) + 130, 500);
			...
			for (var i = 0; i < this._transferLogs.data.length; i++) {
				...
				transferDate[i] = new createjs.Text(newDate, 'bold 12px lato-regular', this.context.theme_color[window.theme].labelcolor);
				transferDate[i].x = 70;
				...
				transferOldCred[i] = new createjs.Text(this.formatNumber(this._transferLogs.data[i].old_money), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				transferAmount[i] = new createjs.Text(this.formatNumber(this._transferLogs.data[i].money), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				transferNewCred[i] = new createjs.Text(this.formatNumber(this._transferLogs.data[i].new_money), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				transferIp[i] = new createjs.Text(this._transferLogs.data[i].ip, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				transferCountry[i] = new createjs.Text(this._transferLogs.data[i].country, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
		...
		displayBetLogs() {
			...
			let betlogsHeader = [
				{"header_width" : 143, "header_name" : window.language.menu.gamenocaps},
				{"header_width" : 163, "header_name" : window.language.menu.datecaps},
				{"header_width" : 133, "header_name" : window.language.menu.tablecaps},
				{"header_width" : 133, "header_name" : window.language.menu.roomcaps},
				// {"header_width" : 143, "header_name" : window.language.menu.startingcreditcaps},
				{"header_width" : 143, "header_name" : window.language.menu.totalbetcaps},
				{"header_width" : 143, "header_name" : window.language.menu.totalwinningscaps},
				// {"header_width" : 143, "header_name" : window.language.menu.newcreditcaps},
				{"header_width" : 143, "header_name" : window.language.menu.resultcaps},
			];
			...
			for (var i = 0; i < betlogsHeader.length; i++) {
			    ...
			    tblBorder[i].graphics.setStrokeStyle(1).beginStroke(this.context.theme_color[window.theme].tbl_border).moveTo(totalWidth, this._tblHeader.y).lineTo(totalWidth, 500);
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				let newCredit = 0;
				let startingCred = data.ante.user_money;
				...
				for (var j in data) {
					...
					if (data[j].win != 0) {
						winMoney = data[j].win + data[j].bet;
						winMoneyCon = data[j].win + data[j].bet;
					}
					else {
						winMoney = data[j].win - data[j].bet;
					}
				...
				betGameNo[i] = new createjs.Text(record.round_num, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				betGameNo[i].x = tblHeader[0].x;
				betGameNo[i].y = (35 * i) + 160;
				betGameNo[i].textAlign = 'center';
				this._betLogsData.addChild(betGameNo[i]);

				betDate[i] = new createjs.Text(newDate, 'bold 12px lato-regular', this.context.theme_color[window.theme].labelcolor);
				betDate[i].x = tblHeader[1].x;
				betDate[i].y = (35 * i) + 160;
				betDate[i].textAlign = 'center';
				this._betLogsData.addChild(betDate[i]);

				betTable[i] = new createjs.Text(record.table_id, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				betTable[i].x = tblHeader[2].x;
				betTable[i].y = (35 * i) + 160;
				betTable[i].textAlign = 'center';
				this._betLogsData.addChild(betTable[i]);

				betRoom[i] = new createjs.Text(record.table_id, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				betRoom[i].x = tblHeader[3].x;
				betRoom[i].y = (35 * i) + 160;
				betRoom[i].textAlign = 'center';
				this._betLogsData.addChild(betRoom[i]);

				// betStartingCred[i] = new createjs.Text(this.formatNumber(startingCred), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				// betStartingCred[i].x = betRoom[i].x + 160;
				// betStartingCred[i].y = (35 * i) + 160;
				// betStartingCred[i].textAlign = 'right';
				// this._betLogsData.addChild(betStartingCred[i]);

				betTotalMoney[i] = new createjs.Text(this.formatNumber(record.total_bet), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				betTotalWinning[i] = new createjs.Text(this.formatNumber(record.total_win), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				...
				// betNewCredit[i] = new createjs.Text(this.formatNumber(newCredit), 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				// betNewCredit[i].x = betTotalWinning[i].x + 160;
				...
				// if void
        if(this._betLogs.data[i].status && this._betLogs.data[i].status.toLowerCase() === 'w') {
          ...
          voidImg.x = voidShape.getBounds().width - 60;
          ...
          voidText.x = voidShape.getBounds().width - 272;
		...
		displayGameHistory() {
			...
			this._histGameNo = new createjs.Text(window.language.menu.gamenocaps, window.language.locale == "zh" ? 'bold 20px lato-regular' : 'bold 13px lato-regular', '#2b2b2b');
			this._histGameNo.x = 100;

			if(window.language.locale == "zh") {
				this._histGameNo.y = this._tblHeader.y + 2;
			} else {
			...
			this._histTableBorder1.graphics.setStrokeStyle(1).beginStroke(this.context.theme_color[window.theme].tbl_border)
						  .moveTo(this._histGameNo.x + 100, this._tblHeader.y).lineTo(this._histGameNo.x + 100, 500);
			this._gameHistAssets.addChild(this._histTableBorder1);

			this._histDate = new createjs.Text(window.language.menu.datecaps, window.language.locale == "zh" ? 'bold 20px lato-regular' : 'bold 13px lato-regular', '#2b2b2b');
			this._histDate.x = this._histGameNo.x + 200;

			if(window.language.locale == "zh") {
				this._histDate.y = this._tblHeader.y + 2;
			} else {
			...
			this._histTableBorder2.graphics.setStrokeStyle(1).beginStroke(this.context.theme_color[window.theme].tbl_border)
						  .moveTo(this._histDate.x + 100, this._tblHeader.y).lineTo(this._histDate.x + 100, 500);
			...
			if (this._histTblElem.length) {
				for (var i = 0; i < this._histTblElem.length; i++) {
					columnName[i] = new createjs.Text(this._histTblElem[i].table_name, window.language.locale == "zh" ? 'bold 20px lato-regular' : 'bold 13px lato-regular', '#2b2b2b');
					columnName[i].x = (this._histTblElem[i].table_width / 2) + prevXCoor;

					if(window.language.locale == "zh") {
						columnName[i].y = this._tblHeader.y + 2;
					} else {
					...
					if (i != this._histTblElem.length - 1) {
						...
						columnBorder[i].graphics.setStrokeStyle(1).beginStroke(this.context.theme_color[window.theme].tbl_border)
							.moveTo(prevXCoor + this._histTblElem[i].table_width, this._tblHeader.y).lineTo(prevXCoor + this._histTblElem[i].table_width, 500);
			...
			for (var i = 0; i < this._gameHistory.data.length; i++) {
				...
				histGameNo[i] = new createjs.Text(this._gameHistory.data[i].round_num, 'bold 13px lato-regular', this.context.theme_color[window.theme].labelcolor);
				histGameNo[i].x = 100;
				histGameNo[i].y = (35 * i) + 160;
				histGameNo[i].textAlign = 'center';
				this._gameHistData.addChild(histGameNo[i]);

				histDate[i] = new createjs.Text(newDate, 'bold 12px lato-regular', this.context.theme_color[window.theme].labelcolor);
		...
		reDrawPagination(record, container, currentArr, lowTenthNum, countTotal, dataCon) {
		  ...
		  let firstPageBtn = new createjs.Shape();
			...
			this._nextIcon.firstPageBtn =nextPageBtn;
		...
		paginate(type, pageNum, container, navigation, callback) {
			let pageUrl = '';
			//Live
			let baseUrl = window.poker_domain + window.tableNum+'/'+range; // +'m/Poker/'

			//Local
			// let baseUrl = 'http://10.1.10.149:8005/'+window.tableNum+'/'+range;
		  ...
		  $.get(pageUrl, {tableId: window.tableNum, bet_type: bet_type, mobile: true}, (response) => {
