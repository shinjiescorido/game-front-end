...
export default () => {
  return new blu.Component({
    main() {
      ...
      // Video
      $('#vidQualityText').html(window.config.video == 'HD' ? window.language2.com_sub_settings_sd : window.language2.com_sub_settings_hd);
      this.toggleQuality(window.config.video);

      $('#menuVidRefresh').click(() => {
        if (this.context.isIpad) {
          this.context.setVideo($('#vidQualityText').html() == window.language2.com_sub_settings_hd ? 'SD' : 'HD');
        } else {
          try {
      ...
      $('#menuChangeQuality').click(() => {
        ...
        $('#vidQualityText').html(newQuality == 'SD' ? window.language2.com_sub_settings_hd : window.language2.com_sub_settings_sd);
