...
let lobby_baccarat_data = {
	...
	showDetails(details) {
		...
		$.post(links.getBaccaratDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
			...
			let winnerTextObj = new createjs.Text(winnerText, '40px LatoBold', '#fff');
		    ...
		    let playerTotalVal = new createjs.Text(parseInt(totalPlayerAmt) % 10, '33px LatoBold', '#fff');
		    ...
		    let bankerTotalVal = new createjs.Text(parseInt(totalBankerAmt) % 10, '33px LatoBold', '#fff');
			...
			for (var i = 0; i < betCount; i++) {
				...
				betTypeTextObj[i].y = betTypeCircle[i].y;
				betTypeTextObj[i].textAlign = 'center';
				betTypeTextObj[i].textBaseline = 'middle';
