...
let lobby_dragontiger_data = {
	...
	showDetails(details) {
		...
		this.tableContainer.x = - 188;
		this.tableContainer.y = - 40;
		...
		this.circleContainer.x = - 95;
		this.circleContainer.y = -20;
		...
		$.post(links.getDragontigerDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
			...
			let winnerTextObj = new createjs.Text(winnerText, '22px LatoBold', '#fff');
			winnerTextObj.x = winnerCircle[0].x;
			winnerTextObj.y = winnerCircle[0].y;
			winnerTextObj.textAlign = 'center';
			winnerTextObj.textBaseline = 'middle';
			...
			if (parityText) {
				let parityTextObj = new createjs.Text(parityText, 'bold 22px LatoBold', '#fff');
				parityTextObj.x = winnerCircle[1].x;
				parityTextObj.y = winnerCircle[1].y;
				parityTextObj.textAlign = 'center';
				parityTextObj.textBaseline = 'middle';
				this.circleContainer.addChild(parityTextObj);

				let sizeTextObj = new createjs.Text(sizeText, 'bold 22px LatoBold', '#fff');
				sizeTextObj.x = winnerCircle[2].x;
				sizeTextObj.y = winnerCircle[2].y;
				sizeTextObj.textAlign = 'center';
				sizeTextObj.textBaseline = 'middle';
			...
			tblBodyBorder.graphics.setStrokeStyle(1).beginStroke('#b7b7b7').drawRect(0, 0, tblWidth - 3, tableHeight);
		    ...
		    for (var j = 0; j < betCount; j++) {
				...
				if (isText) {
					betTextObj[j] = new createjs.Text(betText, '19px LatoBold', '#fff');
					betTextObj[j].x = betCircle[j].x;
					betTextObj[j].y = betCircle[j].y;
					betTextObj[j].textAlign = 'center';
					betTextObj[j].textBaseline = 'middle';
				...
				betAmtObj[j] = new createjs.Text(this.formatNumber(this.betHistory[j].bet_money), 'bold 24px Lato', '#231f20');
				...
				winLoseObj[j] = new createjs.Text(this.formatNumber(this.betHistory[j].win_money), 'bold 24px Lato', winLoseColor);
			...
			let totalBetObj = new createjs.Text(this.formatNumber(totalBetAmt), 'bold 24px Lato', '#242021');
			...
			let totalWinLoseObj = new createjs.Text(this.formatNumber(totalWinLoseAmt), 'bold 24px Lato', winLoseColor);
			...
			let resultValueLeft = new createjs.Text(valDragon.value, '36px LatoBold', '#fff');
			resultValueLeft.x = leftResultCircle.x;
			resultValueLeft.y = leftResultCircle.y;
			resultValueLeft.textAlign = 'center';
			resultValueLeft.textBaseline = 'middle';
			this.cardsContainer.addChild(resultValueLeft);

			let valTiger = cardValue(this.gameInfo.tiger)
			let resultValueRight = new createjs.Text(valTiger.value, '36px LatoBold', '#fff');
			resultValueRight.x = rightResultCircle.x;
			resultValueRight.y = rightResultCircle.y;
			resultValueRight.textAlign = 'center';
			resultValueRight.textBaseline = 'middle';
			...
			let scrollprop = this.scrollbar.scrollable(this.betDetails, this.resultContainer, 1360, 458, true);
