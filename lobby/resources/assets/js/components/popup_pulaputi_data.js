	...
	instance = instance || new blu.Component({
		...
		setBetLogs(data) {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				betDataMarkNum[i] = new createjs.Text(results.num, 'bold 19px Lato', '#000');
				betDataMarkNum[i].x = betDataMarkCircle[i].x + 5;
			...
			$.post(links.getPulaputiDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
				for (var i = 0; i < response.length; i++) {
					...
					for (var j = 0; j < betHistory.length; j++) {
						...
						if (bonusBet == 0) {
							betType[j] = new createjs.Text(betTypeTxt, 'bold 16px Lato', '#231f20');
