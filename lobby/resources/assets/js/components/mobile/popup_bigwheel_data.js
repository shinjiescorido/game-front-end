   ...
   //    		this.makeGameMarks(30);
			...
			$.post("/details/getBigWheelDetails", {round: details.round, table: details.table, betId: details.betId}, (response) => {
				for (var i = 0; i < response.length; i++) {
					...
					for (var j = 0; j < betHistory.length; j++) {
						...
						betMark.y = betMarkTxt.y = (50 * j) + tblHeaderBg.y + 65;
						...
						bets[j].y = (50 * j) + tblHeaderBg.y + 55;
				...
				for (var i = 0; i < betCount + 1; i++) {
				    ...
				    tblBorderHor[i].graphics.setStrokeStyle(1).beginStroke('#b7b7b7').moveTo(tblBodyBorder.x, (tblHeaderBg.y+40) + (i * 50)).lineTo(tblBodyBorder.x + tblWidth, (tblHeaderBg.y+40) + (i * 50));
    ...
    makeGameMarks(size) {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        label[text[i]] = new createjs.Text(text[i], "24px BebasNeue", colors_l[i]);
        ...
        label_latest[text[i]] = new createjs.Text(text[i], "24px BebasNeue", "#000");
