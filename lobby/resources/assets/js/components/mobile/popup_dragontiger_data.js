	...
	instance = instance || new blu.Component({
		...
		setBetLogs(data) {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				betDataMarkNum[i].y = betDataMarkCircle[i].y - 14;
		...
		showDetails(details) {
			...
			$.post(links.getDragontigerDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
				...
				if (parityText) {
					let parityTextObj = new createjs.Text(parityText, 'bold 22px Lato', '#fff');
					...
					let sizeTextObj = new createjs.Text(sizeText, 'bold 22px Lato', '#fff');
				...
				let scrollprop = this.context.lobby_scrollbar.scrollable(this.betDetails, this.resultContainer, 1360, 458, true);
