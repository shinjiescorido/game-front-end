...
export default(links) => {
	instance = instance || new blu.Component({
		...
		main() {
			...
			// this._betLogs = window.pokerLogs;
			// this._betShape = [];
			// this._betText = [];

			// //Pula-Puti log data
			// this.logs = window.pokerLogs;
		...
		setBetLogs(data) {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				betDataMarkNum[i].x = betDataMarkCircle[i].x + 1;
		...
		showDetails(details) {
			...
			detailBg.graphics.setStrokeStyle(2).beginStroke('#000').beginFill('#d5d5d5').drawRoundRect(0, 0, mdlDetailWidth, 760, 7);
			...
			this.cardsContainer.scaleX = this.cardsContainer.scaleY = 1;
			this.betDetails.addChild(this.cardsContainer);
			this.cardsContainer.x = 5;
			this.cardsContainer.y = - 20;
			...
			$.post(links.getBonusPlusDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
				...
				let dealerCardsTxt = new createjs.Text(window.language.poker.dealer, 'bold 22px Lato', '#3a3a3a');
				...
				let communityTxt = new createjs.Text(window.language.poker.communitycard, 'bold 22px Lato', '#3a3a3a');
				...
				let playerCardsTxt = new createjs.Text(window.language.poker.player, 'bold 22px Lato', '#3a3a3a');
				...
				for (var i in this.betHistory) {
					betTypeTxt[count] = new createjs.Text(i.toUpperCase(), 'bold 24px Lato', '#231f20');
					...
					betAmountTxt[count] = new createjs.Text(this.formatNumber(this.betHistory[i].bet), 'normal 24px Lato', '#231f20');
					...
					winLoseAmtTxt[count] = new createjs.Text(this.formatNumber(this.betHistory[i].win), 'normal 24px Lato', moneyColorCode);
				...
				let tblTotalTxt = new createjs.Text(window.language.lobby.total, 'bold 24px Lato', '#231f20');
				...
				let totalBetTxt = new createjs.Text(this.formatNumber(totalBetAmtNum), 'bold 24px Lato', '#231f20');
				...
				let totalWinLoseTxt = new createjs.Text(this.formatNumber(totalWinLoseAmt), 'bold 24px Lato', moneyColorCode);
