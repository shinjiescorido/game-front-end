...
export default(links) => {
	instance = instance || new blu.Component({
		...
		main() {
			...
			// this._betLogs = window.sicboLogs;
			// this._betShape = [];
			// this._betText = [];

			// //Pula-Puti log data
			// this.logs = window.sicboLogs;
		...
		setBetLogs(data) {
			...
			for (var i = 0; i < this._betLogs.data.length; i++) {
				...
				betDataMarkCircle[i].graphics.beginFill('#000').drawCircle(0, 0, 20);
				betDataMarkCircle[i].y = 65 * i;
				this.betResult.addChild(betDataMarkCircle[i]);

				betDataMarkNum[i] = new createjs.Text(markNum, 'bold 23px Lato', '#fff');
			...
			$.post(links.getSicboDetails, {round: details.round, table: details.table, betId: details.betId}, (response) => {
				...
				for (var i = 0; i < response.length; i++) {
					...
					for (var j in this.gameInfo) {
						...
						diceResult[count] = new createjs.Bitmap(this.context.getResources("dice-" + this.gameInfo[j]));
				...
				let winnerTextObj = new createjs.Text(markResultNum, 'bold 30px Lato', '#fff');
				winnerTextObj.x = winnerCircle.x;
				winnerTextObj.y = winnerCircle.y - 18;
				...
				tblTotalText.x = tblHeaderBg.x + 40;
			    ...
			    for (var i = 0; i < betCount; i++) {
			    	betTypeTxt[i] = new createjs.Text(this.betHistory[i].bet.toUpperCase(), 'bold 24px Lato', '#231f20');
					...
					betTypeAmt[i] = new createjs.Text(this.formatNumber(this.betHistory[i].bet_money), 'normal 24px Lato', '#231f20');
					...
					betWinLoseAmt[i] = new createjs.Text(this.formatNumber(this.betHistory[i].win_money), 'normal 24px Lato', moneyColorCode);
				...
				let scrollprop = this.context.lobby_scrollbar.scrollable(this.betDetails, this.resultContainer, 1382, 480, true);
