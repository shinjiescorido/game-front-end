...
export default() => {
	instance = instance || new blu.Component({
		...
		main() {
			...
			confirmationModal = new createjs.Stage("confirmationModal");
			this.container = new createjs.Container();
			confirmationModal.addChild(this.container);

			// this.visible = false;

			this.overlayBg = new createjs.Shape();
			this.overlayBg.graphics.beginFill("rgba(0,0,0,0.5)").drawRect(0,0,1920,1080);
			this.overlayBg.x = 0;
			this.overlayBg.y = 0;
			this.overlayBg.name = "overlay";
			this.container.addChild(this.overlayBg);
		...
		resize () {
			...
			confirmationModal.canvas.width = newWidth;
			confirmationModal.canvas.height = newHeight;


			confirmationModal.scaleX = confirmationModal.scaleY = newWidth / 1920;
		},
		redirectCasinoPage () {
			// this.visible = true;
