    ...
    let betDataMoreInfoText = [];
      ...
      let space = 0;
			...
			for(let j=0;j<results.winner.length;j++) {
				...
				betDataMarkCircle[j].graphics.beginFill(this.bgColor).drawCircle(0, 0, 15);
				if(results.winner.length > 2) {
					betDataMarkCircle[j].x =  space  - 30;
				} else if(results.winner.length == 2) {
					betDataMarkCircle[j].x =  space - 10;
				} else {
					betDataMarkCircle[j].x =  5;
				}
				betDataMarkCircle[j].y = 65 * i;
				this.betResult.addChild(betDataMarkCircle[j]);

				betDataMarkNum[j] = new createjs.Text(totalText, 'bold 19px Lato', '#fff');
				betDataMarkNum[j].y = betDataMarkCircle[j].y - 12;
				betDataMarkNum[j].x = betDataMarkCircle[j].x;
				betDataMarkNum[j].textAlign = 'center';
				//if pair add small circle
				if(isPair) {
					betDataMarkCircle[j].graphics.moveTo(0,15).setStrokeStyle(0.8).beginStroke('#fff').drawCircle(10,10,4);
				}
				...
				if(e.identifier === 'moreInfo') {
					if(e.hitArea && e.hitArea.betId === record.id) {
						if(JSON.parse(record.game_result).winner.length == 1) {
							e.x = (self.betTblHeader[6].x -33) + (JSON.parse(record.game_result).winner.length * 55)
						} else if(JSON.parse(record.game_result).winner.length == 2) {
							e.x = (self.betTblHeader[6].x -33) + (JSON.parse(record.game_result).winner.length * 38)
						} else {
							e.x = (self.betTblHeader[6].x -33) + (JSON.parse(record.game_result).winner.length * 30)
						}
					}

					if(e.betId && e.betId === record.id) {
						if(JSON.parse(record.game_result).winner.length == 1) {
							e.x = (self.betTblHeader[6].x -30) + (JSON.parse(record.game_result).winner.length * 55)
						} else if(JSON.parse(record.game_result).winner.length == 2) {
							e.x = (self.betTblHeader[6].x -30) + (JSON.parse(record.game_result).winner.length * 38)
						} else {
							e.x = (self.betTblHeader[6].x -30) + (JSON.parse(record.game_result).winner.length * 30)
						}
					}
				}
			});
			...
			for (var i = 0; i < response.length; i++) {
				...
				_.forEach(areaArr, (area) => {
					...
					if(!_.isEmpty(tileArea)) {
					  ...
					  winnerTextObj[area] = new createjs.Text(totalText, 'bold 28px Lato', '#fff');
						winnerTextObj[area].x = winnerCircle[area].x;
						winnerTextObj[area].y = winnerCircle[area].y - 19;
