...
let component_createroom = {
  ...
  main (data) {
    ...
    $('#createroom_submit').unbind('click').on('click', (e) => {
      e.preventDefault();
      ...
      if(roomName == "") {
        ...
        $.post(`/setRoomData`, {
          ...
          if(currentOpen == "sicbo") {
            location.replace(window.sb_domain+response.tableId+"/"+response.betRange+"?token="+response.roomToken)
          }

          if(currentOpen == "paigow") {
            location.replace(window.paigow_domain+response.tableId+"/"+response.betRange+"/0?token="+response.roomToken)
          }
