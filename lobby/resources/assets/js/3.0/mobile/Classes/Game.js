...
class Game {
	...
	constructor(data, x, y, w, h, parent, self, isjunket, roomdata) {
		...
		this.canvasCon.className = "game-con-mobile";
		...
		let inputPasswordFailMessage = document.createTextNode('wrong password.');
    ...
    if(window.agent_range.length && !_.isEmpty(a_range)) {
      rangeToUse = a_range.ranges;
		...
		this.timerCanvas.setAttribute('height',`275px`);
		...
		if (window.isJunket > 0) { //if junket
			...
			if(window.isJunket == 2) { //if junket user
				if((data.gameName == 'Baccarat' && auth.baccarat.count > bcCount)
					...
					if(this.roomdata.length) {
						...
						this.createroomButton.className = "createroom createroom--agent disabled-btn gameButtons";
						$(this.canvasCon).append(this.createroomButton);
					}
				...
				if(this.roomdata.length) { //if has rooms
					this.enterButton.className = "enter gameButtons-mobile";
			...
			this.enterButton.className = "enter enter--users gameButtons-mobile";
			$(this.canvasCon).append(this.enterButton);
		}
		...
		// parent
		parent.append(this.canvas, this.timerCanvas, this.canvasHover, this.maintenanceCon);
		...
		// canvas click
		$(this.canvasHover).on('click', (e) =>  {
			$('.menu-con, .menu-content').css('display', '');
		...
		this.bgCon.graphics.beginFill('#2b2b2b').drawRoundRect(0, 0, 915, 275, 0);
		...
		this.gameName = new createjs.Text(headerText,"25px arvoItalic","#000");
		this.gameName.x = 15;
		this.gameName.y = 50/2 + 4;
		...
		if(window.isJunket > 0) {
			...
			if(data.gameName == "Baccarat") {
				this.header.graphics.beginLinearGradientFill(headerColor.baccarat, [0,1], 0, 0, 915, 0).drawRoundRect(0, 0, 915, 50, 0);
			} else {
				this.header.graphics.beginLinearGradientFill(headerColor.main, [0,1], 0, 0, 915, 0).drawRoundRect(0, 0, 915, 50, 0);
			}
			this.gameName.color = "#f1e382"
		}
		...
		if(window.isJunket > 0) {
			if(!_.isEmpty(this.roomdata)) {
				this.status = this.getText(390,35/2, status,fontFormat(25, 'black', 'lato', true),"#000","right","middle");
			} else {
				this.status = this.getText(390,35/2, status,fontFormat(25, 'black', 'lato', true),"#fff","right","middle");
			}
		} else {
			this.status = this.getText(w - 20,50/2, status,fontFormat(25, 'black', 'lato', true),"#fff","right","middle");
		}
		this.stage.addChild(this.status);

		//dealer
		this.dealerBg = new createjs.Shape();
		this.dealerBg.graphics.beginFill("#deca72").drawCircle(0,0,49);
		this.dealerBg.set({x : 67, y : 128})
		this.stage.addChild(this.dealerBg);

		// === dealer name
		this.dealer_name = new createjs.Text(data.currentDealer, "25px lato-black","#fff");
		this.dealer_name.set({x: this.dealerBg.x, y: this.dealerBg.y + 88, textBaseline: 'middle', textAlign: 'center'});
		this.stage.addChild(this.dealer_name);

		//dealer image
		this.dealerImg = new createjs.Bitmap();
		this.dealerImg.set({x:this.dealerBg.x + 1, y: this.dealerBg.y, regX:60, regY:60});
		this.dealerImg.scaleX = this.dealerImg.scaleY = 0.73;
      ...
      if (window.dealerImg[i].id == data.dealerId) {
		...
		this.timer.scaleX = this.timer.scaleY = 0.95;
		this.timer.x = -10;
		this.timer.y = 50;
		...
		if(data.gameName == "Baccarat" && data.type == "flippy") {
			...
			this.flippyImg.scaleX = this.flippyImg.scaleY = 1;
			if(data.roomType == "p") {
				if(window.isJunket == 2) {
					if(!_.isEmpty(this.roomdata)) {
					...
					this.flippyImg.x = this.gameName.getBounds().width + 60;
				...
				if(window.isJunket == 2) {
					...
					this.flippyImg.x = this.gameName.getBounds().width + 40;
				}
			} else {
				this.flippyImg.x = this.gameName.getBounds().width + 30;
			}
		...
		if(!_.isEmpty(this.roomdata)) {
			if(data.gameName == "Baccarat") {
				$(this.canvasOverlay).find('.ico-room-supersix').css({left: this.gameName.getBounds().width + 30})
			}
			if(data.gameName == "Baccarat" && data.type == "flippy") {
		...
		this.roadmapBg.graphics.beginFill("#fff").drawRect(135,50, w, h-85);
		...
		this.resultContainer.x = 135;
		this.resultContainer.y = 50;
		this.stage.addChild(this.resultContainer);

		this.dealingCardAnimationBg = new createjs.Shape();
		this.dealingCardAnimationBg.graphics.beginFill("rgba(22, 22, 22, 0.9)").drawRect(0,0,500,175);
							  ...
							  <div class="popup-body__items -password">
							    ...
							    <div class="password-con input-con">
							      <input type="password" id="junket-${tempdata.gameName}-${tempdata.tableNumber}-roompass" placeholder="${window.language2.nihtanjunket_clientpage_password}" maxlength="4">
									...
									<span class="password error-mes">Password length requirement not met</span>
									<span class="error-empty">Please fill out this field.</span>
							  </div>
							  <div class="popup-body__items -password">
							    ...
							    <div class="password-con input-con">
							      <input type="password" id="junket-${tempdata.gameName}-${tempdata.tableNumber}-confirm-roompass" placeholder="${window.language2.com_sub_rooms_confirmpassword}" maxlength="4">
									...
									<span class="confirm-password error-mes">Password does not match</span>
									<span class="error-empty">Please fill out this field.</span>
							  </div>
							</div>
						`)
		...
		$(document).on("click",`#enter-${this.data.gameName}-${this.data.tableNumber}`, function(e){
				...
				if(window.isJunket == 1 && roomdata.length) {
					// password = $(`#password-${tempdata.gameName}-${tempdata.tableNumber}`).val();
					$.post('/canCreateRoom', { vendor_id: window.vendor_id}, (response) => {
						console.log("response", response[0].flag);
	...
	reposition (x, y) {
		...
		var canvas = document.getElementById(this.namespace);
		canvas.style.left = `${x}px`;
		canvas.style.top = `${y}px`;

		// console.log("reposition",$(this.canvas))
		$(this.canvas).css({
	  	position : 'absolute',
			'margin-top' : 5,
	  	left : `${x}px`,
				...
				if (allgames[i].namespace.split('/')[0] == data.gameName) {
					...
					|| data.gameName == 'Dragon-Tiger' && auth.dragontiger.count > dtCount) {
						if(allgames[i].roomdata.length) {
							...
							$(allgames[i].canvasCon).empty();
							allgames[i].createroomButton.className = "createroom createroom--agent gameButtons";
							allgames[i].canvasCon.append(allgames[i].createroomButton);
						}
					}	else {
						if(allgames[i].roomdata.length > 0) {
							...
							$(allgames[i].canvasCon).empty();
							allgames[i].createroomButton.className = "createroom createroom--agent disabled-btn gameButtons";
							allgames[i].canvasCon.append(allgames[i].createroomButton)
						}
					}
				}
	...
	setMaintenance(data, init) {
		...
		if (maintenance === true && window.userAuthority != 'admin') {
			...
			$(this.timerCanvas).css('visibility', 'hidden');
			$(this.canvas).hide();
			this.isMaintenance = true;
		} else {
			$(this.maintenanceCon).hide();
			$(this.canvasHover).show();
			$(this.timerCanvas).css('visibility', '');
			$(this.canvas).show();
			this.isMaintenance = false;
		}
