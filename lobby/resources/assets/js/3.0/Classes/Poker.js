...
class Poker extends Game {
	constructor(data, x, y, w, h, parent, self, isJunket, roomdata) {
		...
		this.lines.graphics.ss(.8).s("rgba(0,0,0,0.4)").moveTo(160,35)
		this.lines.x = 160;
		this.lines.y = 35;
		...
		//pearl
		for(var i = 0; i <= 6; i++) {
			this.lines.graphics.moveTo(0,30*i).lineTo(240,30*i)
		} // end for

		this.lines.graphics.moveTo(29,0);
		for(var x = 0; x <= 8; x++) {
			this.lines.graphics.moveTo(30*x,0).lineTo(30*x,180)
		}

		this.roadmapData = new createjs.Container();
		this.roadmapData.x = 171;
		this.roadmapData.y = 47;
		...
		this.cardCon.x = 395;
		this.cardCon.y = 50;
		this.roadmap_container.addChild(this.cardCon);
		...
		this.drawRoadMap(fnFormat().fnFormatPokerRoadMap(data.marks,6,7));
		if(window.room_info == 1) {
	...
	displayResult(data) {
		...
		this.drawRoadMap(fnFormat().fnFormatPokerRoadMap(this.data.marks,6,7));
	}
