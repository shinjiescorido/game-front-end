...
class PaiGow extends Game {
    ...
    // setTimeout(() => {
    //
    //     this.dealingCardAnimation.visible = 1
    ...
    //     this.value.scaleX = this.value.scaleY = 1;
    //     this.total.scaleX = this.total.scaleY = 1;
    //     this.value.x = this.total.x + this.total.getMeasuredWidth();
    //     this.textContainer.regX = (this.total.getMeasuredWidth() + this.value.getMeasuredWidth()) /2;
    //     this.textContainer.x = 100
    //     this.stage.update();
    //     console.log(this.dealingCardAnimation,  "::::",  this.textContainer)
    //
    // },2000)
    ...
    this.setNumber = this.getText(850,19, "Set","21px LatoBold","#fff","center","middle");
    // this.roomInfo.addChild(this.setNumber);
    this.data.setNo = this.getText(this.setNumber.x + 35,this.setNumber.y, "0","21px lato-bold","#fff","center","middle");
    // this.roomInfo.addChild(this.data.setNo);
    /*** end room info ***/
    ...
    this.textContainer.addChild(this.value);
		// this.dealingCardAnimation.addChild(this.value);
    ...
    this.animation = [];
		...
		for(var i = 0; i <= 4 ; i++) {
			lines.graphics.moveTo(posX,posY+ (45.9*i)).lineTo(posX + 238.6, posY+ (45.9*i));
		}

		for(var i = 0; i <= 5 ; i++) {
			lines.graphics.moveTo(posX+(48*i),posY).lineTo(posX+(48*i),posY+192);
		}
    ...
    this.roadmapTilesCon.x = 395;
    this.roadmapTilesCon.y = 40;
    this.roadmap_container.addChild(this.roadmapTilesCon);

		for(var i = 0; i < this.TileName.length; i++) {
      let roadmapTiles = new createjs.Container();
      roadmapTiles.x = 10.5 + (i * 127);
      roadmapTiles.y = 28;
  		this.roadmapTilesCon.addChild(roadmapTiles);

			let TilesBg = new createjs.Shape();
			TilesBg.graphics.f(TileBgColor[i]).drawRoundRect( 0, 0, 118, 135, 7 );
    ...
    this.roadmapDataCon.x = 160;
    this.roadmapDataCon.y = 36;
  ...
  getText(x,y,text,font,color,align,valign){
	...
	drawRoadMap(data, isNew) {
        ...
        let circlCon = new createjs.Container();
				...
				circle.graphics.f(area_color).drawCircle(0,0,20);
				circle.x = 22.5 + (i * 48);
				circle.y = 23 + (j * 45);
        ...
        if(areas[j].isWinner) {
          circle.graphics.clear().ss(2).s('gold').f(area_color).drawCircle(0,0,20);
	...
	drawTiles(tile, type) {
      ...
      if(tile[key][0]) {
        ...
        this[key][0].scaleX = this[key][0].scaleY = 1.1;
      ...
      if(tile[key][1]) {
        this[key][1] = createTileSprite(this, 46, 60, this.ctx.load.getResources("small_tiles"));
				this[key][1].scaleX = this[key][1].scaleY = 1.1;
  ...
  inputItem(data) {
    ...
    if(data.type=== 'seat') {
      ...
      // if(data.value == "banker") {
      //   this.textContainer.x = 40;
      // } else if( data.value == "heaven" || data.value == "down") {
      //     this.textContainer.x = 50;
      // }
      // else {
      //   this.textContainer.x = 120;
      // }
      // this.value.x = this.total.x + this.total.getMeasuredWidth() + 5;
      // this.textContainer.regX = this.textContainer.getTransformedBounds().width/2;
      ...
      console.log("::::", this.textContainer.getTransformedBounds().width/2, this.textContainer)
