...
export default(self) => {
    ...
    var socket = io.connect(window.socket+'all', { //io.connect('https://socket.nihtanv2.com/all', { 
    ...
    let init = (data) => {
        ...
        $.post(`/getDealerImg`, {dealerId : self.dealer_id},  (response) => {
            ...
            if (self.lobby_liveGames.all_thumbnial_table.length) {
                for (var i = 0; i < self.lobby_liveGames.all_thumbnial_table.length; i++) {
                    for (var j = 0; j < window.dealerImg.length; j++) {
                        if (self.lobby_liveGames.all_thumbnial_table[i].dealer_id == window.dealerImg[j].id) {
                            let dbImage = new Image();
                            dbImage.src = window.dealerImg[j].dealer_image;
                            self.lobby_liveGames.all_thumbnial_table[i].dealer_image.image = dbImage;
                            break;
                        }
                    }
                }
