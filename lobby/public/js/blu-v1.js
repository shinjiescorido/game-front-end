;var BLU = (function() {
		...
		cycle: {
			...
			run: function() {
				...
				if(this.resources) {
					...
					$.get(this.resources, function(data) {
	 ...
	 */
	BLU.FluidStage = function(canvas, baseWidth, baseHeight) {
	...
	BLU.Screen.prototype.addComponent = function(component) {
		...
		component.main(this.getResources, this);
	};
	...
	/**
	 * XButton is custom button.
	 * @param  {[type]} settings [description]
	 * @return {[type]}          [description]
	 */
	BLU.XButton = function(settings) {
			...
			this.txt.text = settings.text;
			this.txt.textAlign = settings.align;
			this.txt.textBaseline = settings.baseline;
			this.txt.font = settings.textSize + " " + settings.fontFace;
			this.txt.color = settings.color;

			this.btn = BLU.Factory.createButton({
				img: settings.img,
			...
			if(this.btn) {
				this.btn.x = -settings.width / 2;
				this.btn.y = -settings.height / 2;
			}
	...
	createjs.extend(BLU.XButton, createjs.Container);

	BLU.XButton = createjs.promote(BLU.XButton, "Container");
