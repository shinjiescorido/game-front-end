...
class BaccaratController extends Controller
{
    ...
    public function init($table, $gameType = null)
    {
        ...
        // end Check room if normal or created room
        ...
        if(!is_null($usergroup)) {
            ...
            if(!$agentRange->isEmpty()) {
                $tableInfo = DB::connection('baccarat')->table('game_tables')
                    ->where('id', $table)
                    ->get();

                $roomType = $tableInfo[0]->room_type;

                $flippy = json_decode($tableInfo[0]->bet_setting)->type[0];

                $roomType = ($flippy == 'flippy') ? $roomType.'f' : $roomType;
                ...
                for($x = 0; $x < count($agentRange); $x++) {
                    ...
                    if($agentRange[$x]->game === 'Baccarat' && $rtype === $roomType) {
        ...
        if(Auth::user()->is_junket > 0) {
            ...
            for($x = 0;$x < count($checkTable->created_room); $x++) {
                ...
                if($room === $currentTable && Auth::user()->is_junket == 1 && !count($vendorData)) {
                    if($vendor->lobby_type === 'integrated') {
                        return Redirect::to(Auth::user()->pc_redirect_url);
                    } else {
                        return Redirect::to(env('LOBBY_DOMAIN'));
        ...
        if($isMobile) {
            return view('index-mobile', $to_pass_data);
        }
        return view('index', $to_pass_data);
    }
    ...
    public function setRoomInfo(Request $request)
    {
        ...
        DB::select('CALL nihtan_api.USP_ROOMS_SAVE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($flag, 'baccarat', $roomId, $tableId, $roomType, $userId, $status, '', $password, '', 0, 0));
        return $password;
    }
    ...
    public function getVendor($token, $table, $range, $gameType)
    {
        $roomData;
        if ($range == null) {
            $roomData = DB::connection('baccarat')->table("baccarat.rooms")
    ...
    public function getJunketUsers($roomId) {
       return DB::select('CALL baccarat.USP_JUNKET_PAYOUT('. $roomId .')');
    }
    ...
    public function getBetLogs(Request $request)
    {
        ...
        $count = is_null($request->input('count'))  ? 11 : $request->input('count');
        
        return DB::select('CALL baccarat.USP_NEW_GET_BET_HISTORY(?, ?, ?, ?, ?, ?)', array($gameType, $userId, $tableId, $roundNum, $count, $page));
    }

    public function getJunketLogs(Request $request)
    {
        $userId = $request->input('userId');
        $page = $request->input('betPage');
        $tableId = $request->input('tableId');
        ...
        return DB::select('CALL baccarat.USP_GET_JUNKET_BET_HISTORY(?, ?, ?, ?, ?, ?)', array($gameType, (int)$roomId, (int)$userId, (int)$tableId, (int)$roundNum, (int)$page));
    }
    ...
    public function testStore($table, $range, Request $request)
    {
        ...
        $prevTotalBets = $current ? $current->total_bet : 0;
        ...
        app('log')->info(" ================ STORED CHECKINGGGGGGGGG ================== >>>>". $stored[0]->flag);
        if((int)$stored[0]->flag) {
            ...
            app('log')->info(" ================ transactioncheckinggggggggggggggg ================== >>>>". $transaction);
        ...
        if($check ===  true) {
            try {
                ...
                $insertQry = "INSERT INTO bets "
                    ."(`play_type`,`bet_history`, `created_at`, `bet_range`, `room_id`, "
                    . "`round_id`, `type`, `user_id`, `total_bet`, `total_rolling`,"
                    . "`total_winning`, `total_win`, `total_lost`, `device`, `bet_id`, `session_id`, `currency`, `multiplier`) "
    ...
    public function failHandle($type, $prevBets, Request $request, $data)
    {
        ...
        $insertQry = "INSERT INTO bets "
            ."(`play_type`, `bet_history`, `created_at`, `bet_range`, "
            . "`round_id`, `type`, `user_id`, `total_bet`, `total_rolling`, `total_winning`, `total_win`,"
            . " `total_lost`, `device`, `bet_id`, `session_id`, `currency`, `multiplier`) "
            . "VALUES (?, ?, now(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE "
            . "bet_history = ?, total_bet = ?";
    ...
    public function testCancel($table, $range, Request $request)
    {
        ...
        $is_mobile = $request->input('is_mobile') ? true : false;
        $balance = $request->input('balance_bet') ? $request->input('balance_bet')  : 0;
        ...
        $bets = $bet->bet_history;

        if($balance && (int)$balance > 0) {
            $isCancel =  $db->select('CALL USP_BALANCE_CANCEL_CHECK(?, ?, ?, ?)',
               array(
                     Auth::user()->id
                    , $balance
                    , $round->id
