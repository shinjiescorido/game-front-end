...
//--------------------------------------------------------------------------
                if(!isMobile.tablet && !isMobile.phone) {
                    ...
                    self.el.addEventListener("touchstart", function(e) {
                        ...
                        self.boundingRect = self.el.getBoundingClientRect();
                        self.pointers = [0];//e.pointers;
                        self.touchCenter = {x:pos.clientX,y:pos.clientY};//e.center;
                        self.touchRotation = 0;// e.rotation;     

                        self.pointersDown = [0];//e.pointers;
                        self.mouse.position.x = self.mouse.down.x = pos.clientX- self.boundingRect.left;
                        self.mouse.position.y = self.mouse.down.y = pos.clientY - self.boundingRect.top;
                        self.mouse.isDown = true;
                   ...
                   //     mousePos = self.getTouchPos(self.el, e);
                        var touch = e.touches[0];
                        var mouseEvent = new MouseEvent("mousedown", {
                           clientX: touch.clientX,
                            clientY: touch.clientY
                        });
                        self.el.dispatchEvent(mouseEvent);
                    ...
                    self.el.addEventListener("touchmove", function(e) {
                        var touch = e.touches[0];                  
                        self.mouse.position.x = touch.clientX - self.boundingRect.left;
                        self.mouse.position.y = touch.clientY - self.boundingRect.top;   
