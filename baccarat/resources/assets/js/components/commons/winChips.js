...
export default (config) => {
	instance = instance || new blu.Component({
       ...
       $("#test").on("click", () => {
        ...
        this.animateWinLose(area, 'win', {text : '250K'});
        area.loseChips = [];

        this.createWinningChips(area,250000, {}, false);
      })
    },
    animateWinLose(target, type, options) {
      ...
      ((target, posX, posY) => {
        setTimeout(() => {
          ...
          //if has lose chips
          if(target.loseChips !== undefined && target.loseChips.length) {
            target.loseChips.forEach((chip) => {
              createjs.Tween.get(chip, {override: true})
              .wait(waitval)
              .to({
                scaleX: scaleVal,
    ...
    createWinningChips(target, amount, options = {}, anim = true) {
      ...
      let settings = {
        ...
        font2 : fontFormat(15,'normal', 'bebas', false),
      ...
      for (var x = 0; x < chips.length; x++) {
        ...
        if(anim) {
          ...
          target.loseChips.push(chipDataCon);
