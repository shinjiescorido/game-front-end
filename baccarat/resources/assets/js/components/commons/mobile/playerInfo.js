...
export default() => {
	instance = instance || new blu.Component({
		main() {
			// this.user_avatar = new createjs.Bitmap(this.context.getResources("red_queen"));

			// this.user_avatar = new createjs.Bitmap("/img/avatar/poker/"+window.config.default+".png");
			this.bg = new createjs.Shape();
			this.bg.graphics.beginFill(this.context.theme_color[window.theme].base_color).drawRect(-5,10,210,150);
			this.bg.cache(-5,10,210,150);
      ...
      if(textbounds.width > 120) {
        ...
        marquee_con.mask.graphics.beginFill("#fff").drawRect(-65, 0, 120, textbounds.height + 10);
        ...
        this.addChild(this.user_name);
			...
			this.red_bar_text = new createjs.Text("","18px BebasNeue","#fff");
			...
			this.bar_blue.y = 125;
			this.blue_bar_text = new createjs.Text("","18px BebasNeue","#fff");
			this.blue_bar_text.y = 125;
			this.blue_bar_text.x = 5
			this.addChild(this.bar_blue, this.blue_bar_text);

			this.x = this.context.stage.baseWidth - 645;
		...
		cloneUserAvatar(){
			...
			userAvatarContainer.x = this.context.stage.baseWidth - 645;
		...
		changeTheme(new_theme) {
			this.bg.graphics.beginFill(this.context.theme_color[new_theme].base_color).drawRect(-5,10,210,150);
			this.bg.updateCache();
