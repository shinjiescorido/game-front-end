...
// 
					...
					if (this.context.component_betBoard.bet_areas[x].chips.length && !this.context.component_betBoard.bet_areas[x].is_advanced_bet) {
						...
						if (parseInt(this.context.component_betBoard.bet_areas[x].total_bet_amt) < (this.context.component_betBoard.bet_areas[x].min_betAmt)) {
							...
							if(this.context.component_betBoard.bet_areas[x].table_name == "player" || this.context.component_betBoard.bet_areas[x].table_name == "banker") {
								let areas = _.filter(this.context.component_betBoard.bet_areas, function (a) {
									return a.table_name != 'player' && a.table_name != 'banker';
								});

								areas.forEach((a, index) =>{
									this.context.component_chips.removeBetAreaChips(a,index);
								});
								this.yourBets = [];
							}
			...
			this.clearButton.on("mousedown", () => {
				if(this.context.junketAgent) return;
				if(this.context.isBalance && !this.context.component_balanceBet.cancelClickable) return; //for balance bet if upon checking cancel should be disbabled
				if(_.isEmpty(_.find(this.context.component_betBoard.bet_areas, function (e) {return e.chips.length}))) return;
				if(this.process_confirm) return;
			...
			**/
			...
			this.undoButton.on("mousedown", () => {
				...
				if(window.junket == 1 && !_.isEmpty(window.vendorData) && this.context.isBalance) return;
					...
					for(var i =0;i<this.context.component_betBoard.bet_areas.length;i++) {
						if(this.context.multiplayer) {
							if(this.context.component_betBoard.bet_areas[i].table_name == this.previousBet[x].table_id && !this.context.component_betBoard.bet_areas[i].is_advanced_bet && this.context.component_betBoard.bet_areas[i].multiplayer) {
								...
								this.context.component_betBoard.bet_areas[i].total_bet_amt = parseInt(this.previousBet[x].amount);
								...
								if (!is_response) {
									this.context.component_chips.total_ingame_bet += parseInt(this.previousBet[x].amount);
								}
							}
						} else {
		...
		checkButtonState() {
			...
			//confirm check
			if (!this.context.component_timer.betting_start || window.tutorial_enabled) {
				return;
			}
			...
			if(this.context.isBalance && window.junket == 1) {
				this.undoButton.gotoAndStop("disabled");
				this.repeatButton.gotoAndStop("disabled");
				// this.clearButton.gotoAndStop("disabled");
			}
