		...
		dropEvent() {
		  ...
		  let prevBet = null;
  	      ...
  	      return;
				...
				//if has balance bet
				if(window.junket == 1 && !_.isEmpty(window.vendorData) && this.context.isBalance && (e.target.table_name == 'player' || e.target.table_name == 'banker')) {
					areaMaxBet = this.context.component_balanceBet.fnCalcBalanceBet()[e.target.table_name];

					if(this.context.component_gameButtons.yourBets.length) {
						prevBet = _.find(this.context.component_gameButtons.yourBets, function (bets) {return bets.table_id == e.target.table_name})
						if(!_.isEmpty(prevBet)) areaMaxBet += prevBet.amount;
					}
					if(!areaMaxBet) return;
				}
				...
				if ((e.target.total_bet_amt+this.selected_chip.chip_amt) > areaMaxBet) {
					this.context.component_messages.setMessage(window.language.prompts.promptmaxbet,1);
					return;
				} // end if


				if (e.target.total_bet_amt == areaMaxBet) {
					this.context.component_messages.setMessage(window.language.prompts.promptmaxbet,1);
					return;
				} // end if
			...
			if (this.selected_chip.chip_amt == "max") {
				...
				moneyCheck = parseInt(this.context.context.user_money) - this.total_ingame_bet - this.context.component_multibetBetting2.fngetRawTotal(); //additional multibet
	        ...
	        if (parseInt(this.total_ingame_bet)+this.context.component_multibetBetting2.fngetRawTotal() > parseInt(this.context.context.user_money)) {
	         ...
	         e.target.total_bet_amt += parseInt(table_chip.chip_amt);
					...
					e.target.total_bet_amt = areaMaxBet;
            ...
            case "bonus_player":
              ...
              break;
				...
				if(window.multiplayer) {
					let bankerChips = this.context.component_betBoard.bet_areas[11].chips;

					for(let i = 0; i < bankerChips.length; i++) {
						this.context.component_betBoard.setChildIndex(bankerChips[i], this.context.component_betBoard.children.length -1);
					}
				}
