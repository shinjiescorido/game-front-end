...
export default() => {
	instance = instance || new blu.Component({
      ...
      if (window.innerWidth < window.innerHeight && window.matchMedia("(orientation: portrait)").matches) {
        winPosX = 130
        winPosY = this.context.stage.baseWidth - 400;
      } else {
        winPosX = 130
        winPosY = 180;
      }
		...
		giveWinChips(amt) {
			...
			for (var x = 0; x < chips.length; x++) {
				...
				.wait(250)
				.to({
					x : this.context.component_playerInfo.x + 50, //this.winning_chips[x].x + 550,
					y : this.context.component_playerInfo.y //this.winning_chips[x].y + 660,
				},800)
				.wait(150)
				.to({
					alpha : 0
    ...
    screenOrientation() {
      ...
      this.total_win_text.x = winPosX;
      this.total_win_text.y = winPosY;
    }
