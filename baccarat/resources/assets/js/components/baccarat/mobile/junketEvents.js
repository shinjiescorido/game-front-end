...
export default () => {
	return new blu.Component({
		main() {
			...
			$(".room-info,.room-users").on("click", function () {
				$("#balance-info").hide();
				$('.btn-junket').removeClass('active');
				$(this).addClass('active');
				if($(this).hasClass('room-info')) {
					$(".junket-body-info").show();
			...
			$('#changePassYes').off().on('click', (e) => {
				if ($('#inputOldPass').val() == '' || $('#inputNewPass').val() == '' || $('#inputConfirmPass').val() == '') return;
    ...
    placeCaretAtEnd(el) {
		...
		setPlayers() {
			...
			$('.table-td.-hover').off().on('click', (e) => {
				...
				$('.modal-con.modal--betlogs').show();
				$('.modal-con.modal--betlogs').attr('user', $(e.currentTarget).parent().attr('user'));
				$('.modal--betlogs .modal-head-lbl').text(`${window.language.menu.betlogs} (${$(e.currentTarget).parent().children(":first").attr('name')})`);
		...
		setChatBox(e, currentUserId, type = '') {
			...
			let h = '590px';
			if(this.context.portrait) h = '770px';

			$('.junket-chat-con').append(
				`<div id="chatPM${currentUserId}" class="modal-con noselect modal-con-pm modal--chat" user="${currentUserId}">
					<div class="modal-con__head  modal-header-chat" con="chatPM${currentUserId}">
						<span class="modal-head-lbl header-${currentUserId}">${window.language2.com_sub_menuarea_chat} (${pmName})</span>
						<span class="ico-close close-pm" value='chat'></span>
					</div>

					<div class="modal-con__body modal-body-chat --chat" style='height:${h};'>
						<div class="chat-message-con message-con-${currentUserId}" style="height: ${h};">
						</div>
      ...
      if (token == window.vendorData.token) {
		...
		removeNotif(id = false) {
			if (id) {
				...
				$('.menu-chat').removeClass('chatactive');
			}
			if (this.alertInstance) this.alertInstance.stop();
		},
		screenOrientation () {
			if(this.context.portrait) {
				$(".junket-chat-con").addClass('-portrait');
				$(".junket-confirm").addClass('-portrait');
				$("#junket-info").addClass('-portrait');
				$("#junket-agent-info-closebtn").show();
			
      	$(".junket-body-info.-junket-room").hide();
				$(".junket-body-info.-junket-buttons").hide();
				$(".junket-player-con").hide();
      	
				$("#junket-info").hide();
				$("#junket-info").removeClass('acitve');
				
				return
			}
			$(".junket-chat-con").removeClass('-portrait');
			$(".junket-confirm").removeClass('-portrait');
			$("#junket-info").removeClass('-portrait');

			$("#junket-agent-info-closebtn").hide();
			...
			if($(".btn-junket.room-info").hasClass('active')) {
				$(".junket-body-info.-junket-room").show();
				$(".junket-body-info.-junket-buttons").show();
				$(".junket-player-con").hide();
			} else if ($(".btn-junket.room-users").hasClass('active')){
				$(".junket-body-info.-junket-room").hide();
				$(".junket-body-info.-junket-buttons").hide();
				$(".junket-player-con").show();
			}
