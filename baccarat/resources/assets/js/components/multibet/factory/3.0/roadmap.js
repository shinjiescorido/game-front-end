...
class Roadmap {
	constructor(data, el, evt, ctx) {
		...
		this.eye = new createjs.Stage('rm');
		this.eye.x = 0;
		this.self = ctx;
      ...
      $.post("/setGameSettings", {range : range, game: `${this.namespace}`}, function () {
		...
		$("#roadmap-container").css({
				top: calc+'%',
		});

		if(evt.clientX < ($("#myCanvas").width()/2)) {
			$("#roadmap-container").css({
				right: '47.6%',
			});			
		} else {
...
class Baccarat extends Roadmap {
	...
	setRoadmap(data) {
		...
		let marks = fnFormat(slaveJson).fnFormatBCBigRoad(data.marks,6, 26);
		...
		for(var i = 0; i < marks.matrix.length; i++) {
			for(var e = 0; e < marks.matrix[i].length; e++) {
				...
				let sp = null;
				var roadmap = new bcRoadmap(10, 4);
				roadmap.play('big-'+marks.matrix[i][e].mark.toLowerCase(), null, 3);
				roadmap.ties(marks.matrix[i][e].ties, {color:'#000', font: 'italic 16px lato-black', width:4, height:32});
				sp = roadmap.instance;
				sp.x = (e * 27) + 3;
				sp.y = (i * 27.5) + 3;
...
class DragonTiger extends Roadmap {
	...
	setRoadmap(data) {
		this.roadmap_container.removeAllChildren();
		let marks = fnFormat().fnFormatDTBigRoad(data.marks,6, 26);
		...
		for(var i = 0; i < marks.matrix.length; i++) {
			for(var e = 0; e < marks.matrix[i].length; e++) {
				...
				let sp = null;
				var roadmap = new dtRoadmap(10, 4);
				roadmap.play('big-'+marks.matrix[i][e].mark.toLowerCase(), null, 3);
				roadmap.ties(marks.matrix[i][e].ties, {color:'#000', font: 'italic 16px lato-black', width:4, height:32});
				sp = roadmap.instance;
				sp.x = (e * 27) + 3;
...
class Poker extends Roadmap {
	...
	setRoadmap(data) {
		this.roadmap_container.removeAllChildren();
		data = fnFormat().fnFormatPokerRoadMap(data.marks, 6, 23);		
		...
		for(let c = 0; c < data.length; c++ ) {
			...
			for(let n = 0; n < poker_matrix.length; n++ ) {
				...
				sp.graphics.beginRadialGradientFill(["#1565c0","#175296"], [0, 1], 2, 2, 0, 2, 2, 30).drawCircle(0, 0, 11);
				sp.y = (n * 27.5) + 13;
				sp.x = (c *27) + 13;
				let text = new createjs.Text(window.language.locale == "zh" ? "闲" : "P", window.language.locale == "zh" ? "14px latoblack" : "bold 12px Arial", "#ffffff");
				...
				if(poker_matrix[n] == "D"){
						sp.graphics.beginRadialGradientFill(["#d32f2f","#9d2424"], [0, 1], 2, 2, 0, 2, 2, 30).drawCircle(0, 0, 11);
				}
				if(poker_matrix[n] == "T"){
						sp.graphics.beginRadialGradientFill(["#41a257","#41a257"], [0, 1], 2, 2, 0, 2, 2, 30).drawCircle(0, 0, 11);
				}
...
class Sicbo extends Roadmap {
	...
	setRoadmap(data) {
		...
		data = fnFormat().fnFormatSicbo(data.marks, 23, 6).size;
		...
		for(let i = 0; i < data.length; i++) {
		  let size = data[i];
			for(let e = 0; e < size.length; e++) {
				...
				obj.graphics.clear().beginFill('#1665c1').drawCircle(0, 0, 12);

				if(size[e] === 'big') {
					obj.graphics.clear().beginFill('#d32f2e').drawCircle(0, 0, 12);
					data_text.text = 'B'
				} else if(size[e] ==='triple') {
					obj.graphics.clear().beginFill('#689f39').drawCircle(0, 0, 10);
					data_text.text = 'T'
				}
...
class Paigow extends Roadmap {
    ...
    for(var key in this.routeTiles) {
    	...
    	for(var i = 0; i < 4; i++) {
    		...
    		this[key].areas[i].graphics.drawCircle(0,0,5);
    		this.eye.addChild(this[key].areas[i]);
    		this[key].areas[i].y = 145;
    		this[key].areas[i].x = (tempX*14) + this[key].x + 15;
    		tempX ++;
    		if(i >1) {
    			this[key].areas[i].y = 160;
    		}
    		if(i == 1) {
    			tempX = 0;
    		}
    ...
    for (let i=0; i<marks.length; i++) {
      ...
      for(let j=0; j<areas.length; j++) {
        ...
        if(areas[j].isWinner) {
          ...
          winBG.graphics.f('#DEC343').drawRect(0,0,40,40);
          winBG.x = i * 40;
          winBG.y = j * 41.25;
        ...
        circle.graphics.f(areas[j].color).drawCircle(20,20,20);
        circle.x = (i * 40);
        circle.y = (j * 41.25);
        ...
        total.set({textAlign:'center', textBaseline:'middle', x:circle.x + 20, y:circle.y + 20});
